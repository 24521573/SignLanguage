...
class CFG:
    ...
    OUTPUT_ROOT = "/kaggle/working/artifacts/stage5_videomae_large_kinetics_fullframe"
...
def video_name_from_id(video_id: str) -> str:
    text = str(video_id)
    return text if text.endswith(".mp4") else f"{text}.mp4"
...
def build_model(num_classes: int, id_to_class: Dict[int, str]) -> nn.Module:
    ...
    if CFG.USE_GRADIENT_CHECKPOINTING:
        ...
        elif hasattr(model, "videomae") and hasattr(model.videomae, "encoder"):
            model.videomae.encoder.gradient_checkpointing = True
            print("Gradient checkpointing enabled on VideoMAE encoder.")
        else:
            print("Gradient checkpointing requested, but this model does not expose a supported API.")
    return model
...
def predict_dataframe(model: nn.Module, df: pd.DataFrame, with_labels: bool) -> Dict[str, np.ndarray]:
    loader = make_loader(df, train_mode=False, with_labels=with_labels, persistent_workers=False)
    model.eval()
...
def main() -> None:
    ...
    print(
        ...
        f"grad_ckpt={CFG.USE_GRADIENT_CHECKPOINTING}"
    )
    ...
    if len(test_df) > 0:
        ...
        test_pred_df = pd.DataFrame(
            {
                ...
                "video_name": [video_name_from_id(x) for x in test_pred["video_ids"]],
        ...
        submission_df = pd.DataFrame(
            {
                "video_name": test_pred_df["video_name"],
                "label": test_pred_df["pred_class"],
            }
        )
        submission_df.to_csv(submission_path, index=False, encoding="utf-8-sig")
        if CFG.WRITE_ROOT_SUBMISSION:
            root_submission_path = Path("/kaggle/working/submission.csv")
            submission_df.to_csv(root_submission_path, index=False, encoding="utf-8-sig")
        test_summary = {
            ...
            "root_submission_csv": str(root_submission_path) if root_submission_path else "",
        }
    ...
    if len(test_df) > 0:
        ...
        if root_submission_path is not None:
            print(f"Root submission: {root_submission_path}")
